...
# Classements personnels (par periode) du compte Last.fm connecte - a la
# difference de get_top_tracks ci-dessus (playcount Last.fm agrege tous
# utilisateurs, pour un ARTISTE donne), ceux-ci portent sur l'historique
# d'ecoute du compte connecte (methodes user.*), avec un decoupage temporel
# (7day/1month/12month) que Navidrome ne peut pas fournir (voir
# routers/stats.py:favorites) - publiques, cle API seule suffit tant que le
# profil Last.fm cible n'est pas prive.
def get_user_top_artists(username: str, api_key: str, period: str, limit: int = 1) -> list[dict]:
    resp = httpx.get(
        BASE_URL,
        params={
...
def get_user_top_albums(username: str, api_key: str, period: str, limit: int = 1) -> list[dict]:
    resp = httpx.get(
        BASE_URL,
        params={
...
def get_user_top_tracks(username: str, api_key: str, period: str, limit: int = 1) -> list[dict]:
    resp = httpx.get(
        BASE_URL,
        params={
